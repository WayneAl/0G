 ...
 *
 * The reference Agent B runs on the author's laptop behind a quick tunnel, so
 * "offline" is an ordinary state of this system, not a fault of the page. That
 * is why every failure — DNS, CORS, a 502, a body that is not JSON, four
 * seconds of nothing — collapses to the same answer instead of throwing: the
 * pill has one job, and a visitor who cannot reach the auditor still gets a
 * page that verifies seals.
 *
