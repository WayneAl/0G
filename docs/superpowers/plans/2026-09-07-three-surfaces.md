...
**Goal:** Turn the reference pair in `agent-a/` and `agent-b/` into three shells over one core — an SDK on both sides, an MCP server that makes any agent framework an Agent A, and a static website — with seal bodies published to 0G Storage, hosting nothing that holds a key.
...
- **Nothing in this plan deploys or hosts a key.** `agent-a/`, `agent-b/` remain local examples. The only side effects that leave the machine are testnet transactions from the reference A (0G testnet gas, Base Sepolia USDC), and those are run by the orchestrator, never by an implementer.
...
packages/mcp/                         NEW  @acu/mcp
  ...
  src/config.ts · src/server.ts (createServer) · src/tools/*.ts · src/index.ts (stdio bin)
...
web/                                  NEW  @acu/web — Vite multi-page static site
  ...
  index.html · docs/index.html · src/{verify,lookup,render,style}.ts|css · public/directory.json · public/examples/*.json
...
- Produces (`src/config.ts`):
  ...
  export interface McpConfig {
    ...
    auditorUrl: string;               // ACU_AUDITOR_URL, default "http://localhost:4021/audit"
    auditorAgentId: string;           // ACU_AUDITOR_AGENT_ID, default "2"
    directory: Directory;             // from ACU_DIRECTORY_JSON (inline) or fetched from ACU_DIRECTORY_URL at startup; one of the two is required
  ...
  export function loadConfig(env: NodeJS.ProcessEnv, fetchImpl?: typeof fetch): Promise<McpConfig>;  // throws Error("ACU_DIRECTORY_URL or ACU_DIRECTORY_JSON is required")
  ```
- Produces (`src/server.ts`): `export function createServer(config: McpConfig): McpServer` registering exactly these tools (names are the contract):
  ...
  Every handler returns `{ content: [{ type: "text", text: JSON.stringify(result, bigintReplacer, 2) }] }` where `bigintReplacer` renders bigint as string. Paid tools with `agentKey === null` add `"note": "no ACU_AGENT_KEY — stopped at the quote"`. `shareUrl` = `${ACU_WEB_URL ?? "https://wayneal.github.io/0G"}/#seal=${base64url(JSON.stringify(sealA))}` (Task 5's page reads that fragment).
...
- [ ] 6. Commit: `feat: @acu/mcp — six tools that make any agent an Agent A; HttpAgentIdResolver`.

**Wayne-workflow rules for this task:** Verification never crosses the network — `verify_seal` is `@acu/seal` in-process. Keys stay with their owners: the MCP signs with `ACU_AGENT_KEY` and nothing else; never read `.env`. Never the deprecated `.tool()` overloads. stdout is the protocol: no `console.log` anywhere in `packages/mcp/src`. Tests assert real behavior over the real stdio transport.

---

### Task 5: `web/` — Vite static site: verify, look up, docs, pitch, directory

**Files:** Create `web/{package.json,tsconfig.json,vite.config.ts,index.html}`, `web/docs/index.html`, `web/src/{verify,lookup,render,share,main,docs}.ts`, `web/src/style.css`, `web/public/directory.json`, `web/public/examples/{example-sealA,examples}.json` (moved from `verifier/`), `web/scripts/copy-pitch.mjs` · Delete `verifier/` · Modify `.gitignore` (+ `web/dist/`, `web/public/pitch/`), `pnpm-workspace.yaml` (+ `"web"`)
...
- `public/directory.json`: `{ "agents": [ { "agentId": "1", "signer": "<AGENT_A_ADDRESS from the deployed StubVerifier>", "role": "underwriter" }, { "agentId": "2", "signer": "<recover from verifier/example-sealA.json delegations[0].seal>", "role": "auditor", "endpoint": "http://localhost:4021/audit", "price": "$0.01" } ] }`. The implementer derives both addresses with a one-off `tsx` snippet using `recoverSealSigner` on the example seal (A from the outer seal, B from the inner) and records the snippet output in the report — no guessing.
- Pages: `index.html` — three inputs stacked: (1) paste a seal or pick one of the three built-in examples (rail as today), (2) token address → `lookupToken`, (3) shared link auto-loads. Result: verdict banner, seal card(s) walking down into embedded seal B, attestation block, `ON_CHAIN_HASH` row when a lookup was used, "open on storagescan" link (`https://storagescan-galileo.0g.ai/`… link to the tx on `https://chainscan-galileo.0g.ai/tx/${txHash}`). Keep the existing palette and typography from `verifier/index.html` (move its `<style>` into `src/style.css` unchanged). `docs/index.html` — two columns "Be an Agent A" (MCP install block: `claude mcp add acu -e ACU_AGENT_KEY=0x… -e ACU_AUDITOR_URL=… -e ACU_DIRECTORY_URL=<site>/directory.json -- npx -y @acu/mcp`, the six tools, the library snippet `underwrite(req, deps)`) and "Be an Agent B" (`sealedAuditRoute(app, config)` snippet, what the seal carries, the 502 rule); a third section "Verify anywhere" (`verifySealA` snippet, the eight checks). Plain static HTML, same stylesheet.
...
- [ ] 2. `README.md`: update "Repository layout" (add `packages/underwriter`, `packages/auditor`, `packages/storage`, `packages/mcp`, `web/`; remove `verifier/`), add a section **"Use it"** before "Run it" with three sub-sections: *Your agent as an Agent A* (the `claude mcp add` block from Task 5's docs page, the six tools in one table, and a five-line `underwrite()` snippet), *Your x402 service as an Agent B* (`sealedAuditRoute` snippet), *Verify anywhere* (site URL `https://wayneal.github.io/0G/`, `verifySealA` snippet). Update "Verify a seal yourself" to point at the site and the `#seal=` share link. Add "Seal bodies live on 0G Storage" paragraph: what is published, how a reader finds it (Flow `Submit` tags → indexer gateway), the spike tx as the example. Mirror every change in `README.zh-TW.md` in the same register as the existing Chinese (spoken, not translated). Verify: both READMEs mention each of the five package names and the six tool names (`grep -c`).
