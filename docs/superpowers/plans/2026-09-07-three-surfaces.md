...
Task numbers are stable (briefs and commits reference them); the **order is not the numbering**. Dependencies:

`1 → 3 → 2 → 4 → 9 → 5 → 8 → 6 → 7`

- Tasks 1, 3, 2 are done and unaffected by Decision I.
- **Task 4 was already in flight when Decision I landed**, dispatched against the pre-decision text (env-only config, its own `src/usdc.ts`). It is not re-dispatched and not stopped: its six tools, `HttpAgentIdResolver` and the stdio transport are orthogonal to the decision. **Task 9 owns the delta** and runs immediately after it.
- **Task 8 after Task 5**: it rewrites `web/public/directory.json` and extends Task 3's route test.
- Task 6 (docs) after everything it documents; Task 7 (publish prep) last.
...
- **Onboarding is the acceptance.** Every user-facing surface (CLI, MCP, web, README) must work with **no key and no env** up to the quote, and must name the literal next step when it stops.
- **A published package never reads the repo's `.env`.** `dotenv` against `../../.env` is legitimate in `agent-a/` and `agent-b/` (repo-local examples) and nowhere else. `@acu/cli` and `@acu/mcp` read `@acu/config` plus their own environment variables, in that precedence: **env > `~/.acu/config.json` > built-in default.** Refusals a newcomer will hit (`INSUFFICIENT_USDC`, `AUDITOR_UNREACHABLE`, no key) carry the fix in `detail`, not just the code.
...
packages/config/                      NEW  @acu/config — ~/.acu/config.json, 0600, env > file > default
  package.json · tsconfig.json · src/index.ts
  test/config.test.ts                 NEW  precedence, missing file, mode bits (ACU_HOME into a tmpdir)
packages/underwriter/src/status.ts    NEW  agentStatus · usdcBalance · fundingHint (shared by both shells)
packages/cli/                         NEW  @acu/cli — bin `acu`; the headline shell
  package.json (bin: acu) · tsconfig.json · bin/acu.mjs
  src/index.ts                        arg routing + help
  src/commands/{init,status,quote,underwrite,verify}.ts
  src/replay.ts                       MOVED from agent-a/src/replay.ts
  test/{init,args}.test.ts            NEW  init writes 0600 and refuses to clobber; arg parsing
agent-a/src/index.ts                  REWRITTEN again: load repo .env, delegate to @acu/cli's underwrite (output contract unchanged)
...
> **Dispatched 2026-09-07 against the pre-Decision-I text; the text below is the END state, not what the implementer was briefed on.** The in-flight implementation reads config from environment variables only and carries its own `packages/mcp/src/usdc.ts`. Do not re-dispatch or stop it — **Task 9 reconciles it** (delete `usdc.ts`, consume `@acu/underwriter/status`, switch `loadConfig` to `@acu/config` precedence, fix the two onboarding strings). Judge its report against the earlier text; judge the tree against this one.
...
- Produces (`src/config.ts`):
  ...
  export interface McpConfig {
    agentKey: `0x${string}` | null;   // ACU_AGENT_KEY > readUserConfig().agentKey — absent → every paid tool stops at the quote
    ...
    ledgerPath: string;               // ACU_LEDGER_PATH, default `${CONFIG_DIR}/budget-ledger.json` from @acu/config
  ...
  /** Every field resolves env > readUserConfig() > the default named above. Zero env is a supported configuration. */
  ...
  ```
- Consumes (`@acu/underwriter`, created by Task 9): `usdcBalance`, `fundingHint`, `agentStatus` — the MCP does **not** reimplement them; `agent_status` is `agentStatus()` rendered as JSON, so the CLI and the MCP can never disagree.
...
- Produces (`src/server.ts`): `export function createServer(config: McpConfig): McpServer` registering exactly these tools (names are the contract):
  ...
  | `agent_status` | `{}` | `agentStatus()` from `@acu/underwriter`, rendered. Never `isError`. See Task 9 for the payload and the four `nextStep` strings — the no-key one is `"Run: npx @acu/cli init"`, not a re-install of this server. |
  ...
  Every handler returns `{ content: [{ type: "text", text: JSON.stringify(result, bigintReplacer, 2) }] }` where `bigintReplacer` renders bigint as string. Paid tools with `agentKey === null` add `"note": "no key — stopped at the quote. Next: run `npx @acu/cli init`, fund the address it prints, then call this tool again (the MCP reads ~/.acu/config.json; no re-install)."`. `shareUrl` = `${ACU_WEB_URL ?? "https://wayneal.github.io/0G"}/#seal=${base64url(JSON.stringify(sealA))}` (Task 5's page reads that fragment).
...
- [ ] 6. Commit: `feat: @acu/mcp — seven tools that make any agent an Agent A; zero-env config off ~/.acu; HttpAgentIdResolver`.
...
- Pages: `index.html` — **landing-first**, in this order: (0) hero: one sentence ("Your agent hires an auditor. Every hop is signed."), then **two** `<code>` blocks each with a copy button — *See it work*: `npx @acu/cli init` and `npx @acu/cli underwrite 0xDB08Ce217Ce842b06baf76a0Bbb2C10f47fF9eB8`; *Now let your own agent do it*: `claude mcp add acu -- npx -y @acu/mcp` with the caption "no environment variables — it reads the key `acu init` wrote" — and the online/offline pill from `status.ts` fed by `directory.json`'s auditor endpoint (title text says "reference auditor runs on the author's machine; offline means try later"); (1) **feed**: "Seals on chain" — `listRecentListings(…, 20)` cards as above, empty state "Nobody has underwritten yet. Be first." with the install line again; (2) verifier — three inputs stacked: paste a seal or pick one of the three built-in examples (rail as today), token address → `lookupToken`, shared link auto-loads. Result: verdict banner, seal card(s) walking down into embedded seal B, attestation block, `ON_CHAIN_HASH` row when a lookup was used, "open on storagescan" link (`https://storagescan-galileo.0g.ai/`… link to the tx on `https://chainscan-galileo.0g.ai/tx/${txHash}`). Keep the existing palette and typography from `verifier/index.html` (move its `<style>` into `src/style.css` unchanged). `docs/index.html` — opens with **"Try it in 60 seconds"**: the six funnel steps from the spec as a numbered list (`npx @acu/cli underwrite <token>` for the free quote → `npx @acu/cli init` → faucet → `acu status` says Ready → `acu underwrite` → open the `shareUrl` → optionally `claude mcp add acu -- npx -y @acu/mcp` and say it in words), each with its exact command. Then two columns "Be an Agent A" (the five `acu` commands in a table, then the MCP install block `claude mcp add acu -- npx -y @acu/mcp` with no env, `ACU_AGENT_KEY`/`ACU_AUDITOR_URL`/`ACU_DIRECTORY_URL` listed as overrides of `~/.acu/config.json`, the seven tools, the library snippet `underwrite(req, deps)`) and "Be an Agent B" (`sealedAuditRoute(app, config)` snippet, what the seal carries, the 502 rule); a third section "Verify anywhere" (`verifySealA` snippet, the eight checks). Plain static HTML, same stylesheet.
...
- [ ] 2. `README.md`: update "Repository layout" (add `packages/underwriter`, `packages/auditor`, `packages/storage`, `packages/mcp`, `web/`; remove `verifier/`), add a section **"Try it in 60 seconds"** right after the intro (the six funnel steps with exact commands, CLI first; the clone-path variants `node packages/cli/bin/acu.mjs …` and `claude mcp add acu -- node <repo>/packages/mcp/bin/acu-mcp.mjs` in a one-line note until publish), then a section **"Use it"** before "Run it" with three sub-sections: *Your agent as an Agent A* (the `acu` command table, the `claude mcp add` block from Task 5's docs page, the seven tools in one table, and a five-line `underwrite()` snippet), *Your x402 service as an Agent B* (`sealedAuditRoute` snippet, prefaced with "once there are As — the reference auditor is the only B today"), *Verify anywhere* (site URL `https://wayneal.github.io/0G/`, `verifySealA` snippet). Update "Verify a seal yourself" to point at the site and the `#seal=` share link. Add "Seal bodies live on 0G Storage" paragraph: what is published, how a reader finds it (Flow `Submit` tags → indexer gateway), the spike tx as the example. Mirror every change in `README.zh-TW.md` in the same register as the existing Chinese (spoken, not translated). Also a paragraph *The reference auditor runs on the author's machine* (why: keys stay with owners, nothing deployed; how: `demo/serve-b.sh`; what you see when it is off: the site pill and `AUDITOR_UNREACHABLE`). Verify: both READMEs mention each of the seven package names and the seven tool names (`grep -c`), and "Try it in 60 seconds" appears before "Run it".
...
**Files:** Modify `packages/{seal,og,underwriter,storage,auditor,config,cli,mcp}/package.json`, add `packages/*/tsconfig.build.json`, `packages/mcp/bin/acu-mcp.mjs` (switch to `dist/`), root `package.json` (`build` script already runs `pnpm -r build`)

**Interfaces:** none new. Each package gains `"build": "tsc -p tsconfig.build.json"` emitting ESM + `.d.ts` to `dist/`, `"files": ["dist", "README.md"]`, `"publishConfig": { "access": "public" }`, and `exports` that map to `dist/*.js` **under a `publishConfig.exports`** override so the workspace keeps resolving `src/*.ts` (pnpm applies `publishConfig` at pack time). `private: true` removed from the eight publishable packages (`seal`, `og`, `underwriter`, `storage`, `auditor`, `config`, `cli`, `mcp`); `agent-a`, `agent-b`, `demo`, `web` stay private. `@acu/cli` bin → `dist/index.js` alongside the MCP's. `@acu/mcp` bin → `dist/index.js` with the shebang preserved.
...
- [ ] 2. `mkdir -p .claude/sdd/pack && pnpm -r --filter './packages/*' exec pnpm pack --pack-destination "$PWD/.claude/sdd/pack"` (gitignored) → eight tarballs; `tar tzf` one and confirm `dist/index.js`, `dist/index.d.ts`, `package.json` present and no `src/`. Report the tarball sizes.
- [ ] 3. Commit: `chore: build + publishConfig for the eight publishable packages (publish held)`.
...
- `demo/serve-b.sh` (orchestrator-run; reads `.env`): requires `cloudflared` on PATH (prints `brew install cloudflared` and exits 1 otherwise); starts the reference B (`pnpm --filter @acu/agent-b start`) on `:4021`; runs `cloudflared tunnel --url http://localhost:4021` with its output redirected to a log file, waits for the origin, calls `set-directory-endpoint.mjs` with it, prints the origin and `GET <origin>/agent` once as a liveness proof, then waits on both children; `trap` on exit kills both and **restores** `directory.json` to `http://localhost:4021` so the committed file never points at a dead tunnel. Flag `--keep` skips the restore for the demo-day commit.

**Verified by the orchestrator (2026-09-07, cloudflared 2026.8.3 from Homebrew — no account, no plugin, no config file):** a quick tunnel to a local server answered `HTTP/2 200` end to end and passed `access-control-allow-origin: *` through unchanged, so the site's pill will work once Step 1 lands. Facts the script must be built on, all observed:
- The origin is printed **to stderr** inside a boxed banner line (`|  https://<three-or-four-words>.trycloudflare.com  |`), about **4 s** after launch, and **before** the connectivity-precheck summary — so wait on the URL pattern, never on a "healthy" line. Redirect with `> "$LOG" 2>&1` and parse with `grep -oE 'https://[a-z0-9-]+\.trycloudflare\.com' "$LOG" | head -1`.
- The wait must be bounded: poll the log every 0.5 s for at most 60 s, then print the last 20 log lines and exit 1. Never loop forever, and never busy-wait without a sleep.
- Subdomain words vary per run, so the origin rotates on every restart; that is why `directory.json` is rewritten each time rather than committed.
...
- [ ] 4. **Orchestrator acceptance (held, reads `.env`, moves testnet funds):** `./demo/serve-b.sh` → origin printed and `/agent` answers. Then, on a clean shell with **no repo env** and `ACU_HOME` pointed at a throwaway directory (so the real `~/.acu` is untouched), in funnel order: (i) `node packages/cli/bin/acu.mjs underwrite 0xDB08…9eB8` with no key → the 402 quote and "Run: npx @acu/cli init"; (ii) `acu init` → address + faucet URL, `~/.acu/config.json` is `0600`; (iii) `acu status` → "Fund …"; (iv) faucet; (v) `acu status` → "Ready"; (vi) `acu underwrite` → seal A, `shareUrl` opens green, the feed shows the listing; (vii) `claude mcp add acu -e ACU_DIRECTORY_JSON="$(cat web/public/directory.json)" -- node $PWD/packages/mcp/bin/acu-mcp.mjs` — **no key in the command** (the `ACU_DIRECTORY_URL` default only works once Pages is live; `ACU_DIRECTORY_JSON` stands in) — then in a fresh Claude Code session `agent_status` reports Ready off the same file and "underwrite CleanUSD" produces a second seal. Also assert the non-reference path: a second burner key gets `listing: null, skipped.settle`. Record wall-clock from step (i) to the green page; target ≤ 5 min. Commit `directory.json` with the live tunnel only for the demo (`--keep`), and note in the README that the URL rotates.
...
### Task 9: `@acu/config` + `@acu/cli` — the headline shell, and the shared key both shells read

**Runs immediately AFTER Task 4** (which shipped before Decision I landed; this task reconciles it). Decision I in the spec.

**Files:** Create `packages/config/{package.json,tsconfig.json}`, `packages/config/src/index.ts`, `packages/config/test/config.test.ts` · Create `packages/underwriter/src/status.ts` (+ export from `src/index.ts`), `packages/underwriter/test/status.test.ts` · Create `packages/cli/{package.json,tsconfig.json}`, `packages/cli/bin/acu.mjs`, `packages/cli/src/index.ts`, `packages/cli/src/commands/{init,status,quote,underwrite,verify}.ts`, `packages/cli/test/{init,args}.test.ts` · Move `agent-a/src/index.ts` → `packages/cli/src/commands/underwrite.ts` and `agent-a/src/replay.ts` → `packages/cli/src/replay.ts` · Rewrite `agent-a/src/index.ts` as the shim · Modify `agent-a/package.json`, `pnpm-workspace.yaml` (nothing to add — `packages/*` already globs) · **Retrofit Task 4:** delete `packages/mcp/src/usdc.ts`, modify `packages/mcp/src/config.ts`, `packages/mcp/src/tools/agent-status.ts`, `packages/mcp/src/tools/{hire-audit,underwrite}.ts`, `packages/mcp/package.json`, `packages/mcp/test/tools.test.ts`

**Interfaces:**
- Produces (`packages/config/src/index.ts`):
  ```ts
  export interface UserConfig {
    agentKey: `0x${string}` | null; agentId: string | null; directoryUrl: string | null;
    auditorUrl: string | null; registry: `0x${string}` | null; ledgerPath: string | null; webUrl: string | null;
  }
  /** process.env.ACU_HOME ?? path.join(os.homedir(), ".acu") — read fresh on every call, so tests can set ACU_HOME. */
  export function configDir(env?: NodeJS.ProcessEnv): string;
  export function configPath(env?: NodeJS.ProcessEnv): string;   // <configDir>/config.json
  /** Missing file or missing directory → every field null. Never throws for absence. Malformed JSON throws Error("CONFIG_MALFORMED: <path> — <cause>"); an unreadable file throws. Unknown keys are preserved on write but ignored on read. */
  export function readUserConfig(env?: NodeJS.ProcessEnv): UserConfig;
  /** Merges patch over the file, creates the dir 0700 and the file 0600, writes atomically (tmp + rename). Returns the merged config. */
  export function writeUserConfig(patch: Partial<UserConfig>, env?: NodeJS.ProcessEnv): UserConfig;
  /** env > file > fallback, in that order. Empty string in env counts as unset. */
  export function resolve<T>(envValue: string | undefined, fileValue: T | null, fallback: T): T;
  ```
  No dependency other than node builtins — the MCP, the CLI and any future shell all import this and nothing else for user config.
- Produces (`packages/underwriter/src/status.ts`) — the one implementation behind `acu status` and the MCP's `agent_status`:
  ```ts
  ...
  /** balanceOf(payer). Throws Error("USDC_RPC_UNREACHABLE: …") on transport failure — never returns 0 for a failed read. */
  export async function usdcBalance(opts: { usdc?: `0x${string}`; rpcUrl?: string; client?: PublicClient }, payer: `0x${string}`): Promise<bigint>;
  /** "Fund <payer> with Base Sepolia USDC at <faucetUrl> (choose Base Sepolia, paste the address). Need <need>, have <have>." */
  export function fundingHint(payer: `0x${string}`, needAtomic: bigint, haveAtomic: bigint, faucetUrl?: string): string;
  export interface AgentStatus {
    agentId: string; address: `0x${string}` | null; hasKey: boolean;
    ...
    directoryUrl: string | null; nextStep: string;
  }
  export async function agentStatus(deps: { … }): Promise<AgentStatus>;
  ```
  `nextStep` is exactly one of, evaluated in this order:
  1. `"Run: npx @acu/cli init"` — no key. (Not a re-install of anything; this is the whole point of Decision I.)
  2. `"Reference auditor is offline — try again later, or point at another one with ACU_AUDITOR_URL"` — auditor unreachable.
  3. the `fundingHint` text — key present, auditor online, balance below the auditor's price (below `$0.01` when the price is unknown).
  4. `"Ready: acu underwrite 0xDB08Ce217Ce842b06baf76a0Bbb2C10f47fF9eB8"`.
- Produces (`packages/cli/src/index.ts`): `export async function main(argv: string[]): Promise<number>` — routes the five commands, prints help on none/`--help`/unknown, returns the exit code (the bin calls `process.exit`). Commands:
  | command | behavior |
  |---|---|
  | `acu init` | `generatePrivateKey()` from `viem/accounts` unless `--key 0x…`; **refuses** with exit 1 if `agentKey` is already set and `--force` is absent, printing the existing address; `writeUserConfig({ agentKey, agentId })`; prints the address, `configPath()`, the faucet URL, and `Next: acu status`. Prints the line `this is a burner key for testnet — never fund it with real money` every time. |
  | `acu status` | `agentStatus(...)` printed as aligned lines, `nextStep` last and highlighted. `--json` prints the object. |
  | `acu quote <token>` | `underwrite` with `dryRun: true`; the quote, the budget line, `nextStep`. |
  | `acu underwrite <token>` | the moved printer. **Output contract frozen** (see Global constraints): the `[N] ` step lines, and a last line matching `^(✓ [A-Z_]+|✗ [A-Z_]+|○ DRY RUN|○ NOT LISTED)`. Flags unchanged: `--ltv --live --no-settle --publish --endpoint --registry --source --emit-seal --seal-file --offline`. On success also prints the `shareUrl`. |
  | `acu verify <file\|->` | reads a seal from a file or stdin, `verifySealA`/`verifySealB` by `type`, prints the check rows, exit 0 valid / 1 invalid. Local only, no network beyond agent-id resolution. |
  Config resolution in every command: `env > readUserConfig() > default`, via `@acu/config`. `@acu/cli` must not import `dotenv`.
- Produces (`packages/cli/bin/acu.mjs`): the same `tsx/esm` shim shape as the MCP's bin, `"bin": { "acu": "./bin/acu.mjs" }`, `tsx` a runtime dependency. Task 7 repoints it at `dist/`.
- Produces (`agent-a/src/index.ts`, ~6 lines):
  ```ts
  // The reference Agent A is a shim over @acu/cli. The repo .env is read HERE and never inside the published package.
  import { config as loadEnv } from "dotenv";
  loadEnv({ path: new URL("../../.env", import.meta.url).pathname });
  const { main } = await import("@acu/cli/underwrite");
  process.exit(await main(process.argv.slice(2)));
  ```
  `agent-a/package.json` keeps `start`, gains `@acu/cli` as a dependency, and drops the deps that moved.

**Steps:**
- [ ] 1. RED `packages/config/test/config.test.ts` with `ACU_HOME` set to a `mkdtemp` dir: missing file → all nulls; `writeUserConfig({agentKey})` then `readUserConfig()` round-trips; the file's mode is `0600` and the dir's `0700` (`statSync(...).mode & 0o777`); a second write merges rather than replaces and preserves an unknown key written by hand; malformed JSON → throws `CONFIG_MALFORMED`; `resolve("env", "file", "dflt") === "env"`, `resolve(undefined, "file", "dflt") === "file"`, `resolve("", "file", "dflt") === "file"`, `resolve(undefined, null, "dflt") === "dflt"`. Run → FAIL. GREEN: implement. `pnpm --filter @acu/config test` green.
- [ ] 2. RED `packages/underwriter/test/status.test.ts`: `fundingHint` contains the payer, the faucet URL and "Base Sepolia"; `usdcBalance` with an injected client returning `0n` → `0n`, and with a throwing client → rejects with `USDC_RPC_UNREACHABLE` (never `0n`); `agentStatus` with injected seams returns each of the four `nextStep` strings for the four states, **in the documented precedence** (no key beats offline auditor; offline auditor beats an empty balance). Run → FAIL. GREEN: `status.ts` + export. `pnpm --filter @acu/underwriter test` green (31 + new).
- [ ] 3. CLI skeleton + the move. `git mv agent-a/src/index.ts packages/cli/src/commands/underwrite.ts && git mv agent-a/src/replay.ts packages/cli/src/replay.ts`, then strip the `dotenv` import and the `loadEnv(...)` call from the moved file, convert its `process.exit(...)` tail into `return <code>` from an exported `main(argv)`, and switch its config reads to `@acu/config` + env. Write the `agent-a/src/index.ts` shim. Keep every printed string byte-identical. `pnpm install`.
- [ ] 4. RED `packages/cli/test/{init,args}.test.ts` with `ACU_HOME` in a tmpdir: `main(["init"])` → exit 0, config written `0600`, stdout has the address and the faucet URL and the burner-key warning; a second `main(["init"])` → exit 1, stdout names the existing address, the key on disk **unchanged**; `main(["init","--force"])` → new key, different address; `main(["init","--key","0x"+"11".repeat(32)])` → that key's address; `main([])` and `main(["nope"])` → help, exit 2 for the unknown command; `main(["verify", <path to verifier/example-sealA.json>])` → exit 0; against `examples.json`'s `tampered` → exit 1 and `SIGNER_MISMATCH` in the output. Capture stdout by injecting a writer, not by patching the global console. Run → FAIL. GREEN.
- [ ] 5. Verification commands: `ACU_HOME=$(mktemp -d) node packages/cli/bin/acu.mjs init` → address + faucet + `0600`; `ACU_HOME=$(mktemp -d) node packages/cli/bin/acu.mjs status --json | head -40` → JSON whose `nextStep` is `"Run: npx @acu/cli init"`; `node packages/cli/bin/acu.mjs --help` → the five commands; `pnpm -r typecheck && pnpm -r test` green. **`./demo/run.sh --offline` must still print the same seven-scene assertions** — that is the frozen output contract surviving the move, and it is the point of this task's verification. (`run.sh --offline` touches no network and no `.env` values that matter; it is the one runner an implementer may execute. Any other mode stays with the orchestrator.)
- [ ] 6. **Retrofit the MCP (Task 4 predates Decision I).** Read `packages/mcp/src/` first and keep its structure. (a) `loadConfig` resolves every field `env > readUserConfig() > default` via `@acu/config`; add `@acu/config` to its deps. (b) Delete `packages/mcp/src/usdc.ts` and import `usdcBalance` / `fundingHint` from `@acu/underwriter` instead — one implementation, or the CLI and the MCP will drift. (c) `agent-status.ts` becomes a renderer over `agentStatus()`; its no-key `nextStep` becomes `"Run: npx @acu/cli init"`. (d) The paid-tool note becomes the no-re-install wording in Task 4's interface table. (e) Add to `packages/mcp/test/tools.test.ts`: with `ACU_HOME` in a tmpdir holding a `config.json` with an `agentKey` and **no** `ACU_AGENT_KEY`, the key comes from the file; with both set, env wins. Verify: `pnpm --filter @acu/mcp test` green, and `echo '{...initialize...}' | ACU_DIRECTORY_JSON='{"agents":[]}' node packages/mcp/bin/acu-mcp.mjs` still answers with `serverInfo.name === "acu"` **with no other environment variable set**.
- [ ] 7. Commits: `feat: @acu/config — ~/.acu/config.json, 0600, env > file > default`; `feat: underwriter — agentStatus, usdcBalance, fundingHint shared by both shells`; `refactor: @acu/cli — the reference A becomes a publishable CLI; agent-a is a shim`; `refactor: mcp — read ~/.acu/config.json, share the underwriter's status; zero-env install`.

**Wayne-workflow rules for this task:** The MCP retrofit is a **surgical edit of someone else's finished task** — read it before changing it, keep its structure and its tests, change only the four things step 6 names. The moved printer is a **move**, not a rewrite — every user-visible string stays byte-identical or `demo/run.sh` breaks, and run.sh is the regression test. A published package never reads the repo `.env`; that line lives in the `agent-a` shim only. The config file holds a private key: `0600`, `0700`, atomic write, and `init` refuses to clobber without `--force` — losing a funded burner key to a careless re-run is the failure this prevents. Never print the private key, in any command, including `status --json`. Fail loud: `usdcBalance` on a dead RPC raises, it does not report a zero balance that would send the user to the faucet for nothing.
